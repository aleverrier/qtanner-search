...
from qtanner.classical_distance import analyze_parity_check_bitrows
...
def main() -> int:
    ...
    C1 = _apply_variant(base_code, args.a1v)
    C1p = _apply_variant(base_code, args.b1v)
    ...
    hx_path = out_dir / "Hx.mtx"
    hz_path = out_dir / "Hz.mtx"
    ...
    dx_stats = analyze_parity_check_bitrows(
        hx_rows, n_cols, max_k_exact=18, samples=2000, seed=args.seed
    )
    dz_stats = analyze_parity_check_bitrows(
        hz_rows, n_cols, max_k_exact=18, samples=2000, seed=args.seed
    )
