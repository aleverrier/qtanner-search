...
OUTDIR="$HOME/research/qtanner-search/results/bench_dist_$TS"
mkdir -p "$OUTDIR"
...
echo "STEPS=$STEPS WMIN=$WMIN SEED=$SEED WMAX=$WMAX" | tee -a "$OUTDIR/runinfo.txt"
...
if [ "${#CODES[@]}" -lt 2 ]; then
  echo "ERROR: Need at least two HX_*.mtx files in $CODEDIR (or pass two suffixes as args)" >&2
  exit 1
fi
...
run_one_code() {
  ...
  local log_cc_z="$OUTDIR/${name}_distm4ri_CC_Z.txt"
  local log_cc_x="$OUTDIR/${name}_distm4ri_CC_X.txt"
  local log_gap="$OUTDIR/${name}_qdistrnd.txt"
  local tim="$OUTDIR/${name}_timing.txt"

  echo "dist-m4ri RW Z" | tee -a "$tim"
  ...
  echo "dist-m4ri RW X" | tee -a "$tim"
  /usr/bin/time -p -o /tmp/tim.$$ \
    "$DISTBIN" debug=0 method=1 finH="$HZ" finG="$HX" steps="$STEPS" wmin="$WMIN" seed="$SEED" \
    | tee "$log_rw_x" >/dev/null
  sed 's/^/  /' /tmp/tim.$$ | tee -a "$tim"

  echo "dist-m4ri CC Z (wmax=$WMAX)" | tee -a "$tim"
  /usr/bin/time -p -o /tmp/tim.$$ \
    "$DISTBIN" debug=0 method=2 finH="$HX" finG="$HZ" wmax="$WMAX" \
    | tee "$log_cc_z" >/dev/null
  sed 's/^/  /' /tmp/tim.$$ | tee -a "$tim"

  echo "dist-m4ri CC X (wmax=$WMAX)" | tee -a "$tim"
  /usr/bin/time -p -o /tmp/tim.$$ \
    "$DISTBIN" debug=0 method=2 finH="$HZ" finG="$HX" wmax="$WMAX" \
    | tee "$log_cc_x" >/dev/null
  sed 's/^/  /' /tmp/tim.$$ | tee -a "$tim"
...
Print("Code: $name\\n");
Print("Running DistRandCSS(HX,HZ,num=$STEPS,mindist=0,debug=0 : field:=GF(2))\\n");
t0 := Runtime();
dz := DistRandCSS(HXm, HZm, $STEPS, 0, 0 : field := GF(2));
t1 := Runtime();
Print("dZ = ", dz, "   time_ms=", (t1-t0), "\\n");

Print("Running DistRandCSS(HZ,HX,num=$STEPS,mindist=0,debug=0 : field:=GF(2))\\n");
t0 := Runtime();
