...
- Typical invocation: `python3 scripts/search_progressive.py ...`
- Alternate entry point: `python -m qtanner.search progressive ...` (dispatches to the same progressive implementation)
...
- Steps:
  ...
  3. Runs `python -u scripts/search_progressive.py ...` and logs to `logs/`.
...
- Steps:
  ...
  2. Runs `python3 scripts/refine_best_codes_length.py --n N --trials-per-side T ...`.
  3. Runs `python3 scripts/rebuild_best_codes_artifacts_from_meta.py --best-dir best_codes`.
  4. Runs `python3 scripts/sync_best_codes_matrices.py --best-dir best_codes`.
  5. Commits + pushes updates.

### Alternate pipeline
- Script: `scripts/refine_length_m4ri_pipeline.sh`
- Steps:
  1. Runs `python3 scripts/refine_best_codes_m4ri_by_length.py ...`.
...
- Steps:
  1. Runs `python scripts/collect_best_codes.py` (collects results/**/best_codes into best_codes/).
  2. Runs `python scripts/generate_best_codes_site.py` (regenerates best_codes/data.json).
  3. `git add` + commit + push.

### Publish for a group/order
- Script: `scripts/publish_best_codes_group.sh`
- Steps:
  1. Runs `python3 scripts/publish_best_codes_group.py ...` to merge results into best_codes/.
  2. Runs `python3 scripts/prune_best_codes_group.py ...` for each matching group.
  3. Runs `python3 scripts/rebuild_best_codes_artifacts_from_meta.py --best-dir best_codes`.
  4. Runs `python3 scripts/sync_best_codes_matrices.py --best-dir best_codes`.
  5. Commits + pushes.
...
- Steps:
  1. Runs `python3 scripts/rebuild_best_codes_artifacts_from_meta.py`.
